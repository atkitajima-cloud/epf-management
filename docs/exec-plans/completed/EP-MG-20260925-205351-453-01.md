...
- `backlog`、`ready`、`doing`、`review`のTaskは削除できる。
- `done`のTaskは完了・受入記録を保持するため削除できない。APIは拒否し、画面も削除操作を表示しない。
- 誤って完了したTaskは削除ではなく、既存の状態更新で`done`以外へ戻してから削除する。
...
- `npm test`: 49件成功。
- `node scripts/check-records.mjs`: エラー0件、切れたリンク0件。
- 実装結果の人間レビュー待ち。
