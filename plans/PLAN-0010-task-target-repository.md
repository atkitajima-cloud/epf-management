...
- `target_repo` をTask正本フィールドとして追加し、4repoと `common` のみを受け入れる検証を実装した。
- 作成・編集ダイアログ、Kanbanカード、ガントの表示・絞り込み、WBS生成を対応させた。Kanbanとガントでは短縮名を色付きバッジで表示する。
- 既存22件のTaskを移行し、旧 `frontend_repo` / `backend_repo` を除去した。既存の明示的なrepo関連付けとTask本文の対象repoを引き継ぎ、それ以外は `common` とした。
- JavaScript構文検証、Task読込検証、`npm test` を実行し、25件のテストがすべて成功した。
- 実行環境に利用可能なブラウザがなかったため、画面での手動操作確認はレビュー時に実施する。
- 2026-09-21: ブラウザでKanbanカードを確認し、対象リポジトリの表示に問題がないことを確認した。
